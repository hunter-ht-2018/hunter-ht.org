...
def home(request):
    ...
    if not Publications.objects.last():
        ...
        if Publications.objects.last().pubID > 0:
            ...
            articles = Articles.objects.filter(publish="1", isPrivate='0').order_by("editDateTime").reverse()
            ...
            labMembers = User.objects.filter(identity="0").all();
...
def operator(request):
    ...
    if request.is_ajax():
        ...
        if actiontype == '4':
            ...
            destination = os.path.join(BASE_DIR, 'web_src', 'static', 'publications', pubFileName)
            try:
...
def index(request):
    if request.method=="GET":
        if 'user' in request.GET:
            ...
            articles = Articles.objects.filter(Q(authorID=userID) | Q(isPrivate='1', publish='1')).order_by("editDateTime").reverse()
        ...
        if 'articleID' in request.GET:
            ...
            authorname = User.objects.get(userID=authorID).name
            # jsonStr = '{"title":"'+article.title+'","content":"'+article.content+'","authorname":"'+authorname+'"}'
            response = JsonResponse({"title": article.title,"content":article.content,"authorname":authorname})
            return response
            # return HttpResponse(json.dumps(jsonStr), content_type='application/json')
    if request.is_ajax():
    ...
    if request.POST:
        ...
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        destination = os.path.join(BASE_DIR,'web_src', 'static', 'publications',myfile.name)
        if os.path.exists(destination):
            os.remove(destination)
...
def write(request):
    ...
    if request.is_ajax():
        ...
        isPrivate = User.objects.get(name=username).identity
        print "是否为内部通知"
        print isPrivate
        ...
        try:
            ...
            if not authorID:
                ...
                try:
                    Articles.objects.create(articleID=articleID, authorID=authorID, title=title, content=content,publish=publish,editDateTime=editdatetime,isPrivate = isPrivate)
                    if publish == '0':
...
def uploadImg(request):
    ...
    BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    file_path = os.path.join(BASE_DIR, 'web_src', 'static', 'articles','mdImage', file_obj.name)
